...
*************************************************** **/
	if(jQuery("#iscrollWrapper").length > 0) {
		...
		var _shopWidth	= _shopItems * _itemWidth + (20 * parseInt(_shopItems));
		var _shopHeight	= jQuery("#scroller .item").height();
		if(_shopItems > 8) {
			_shopWidth = Math.ceil(parseInt(_shopItems) / 1) * parseInt(_itemWidth) + (10 * parseInt(_shopItems)); //Math.ceil(parseInt(_shopWidth) / 2);
			_shopHeight = parseInt(_shopHeight) * 1;
		}
